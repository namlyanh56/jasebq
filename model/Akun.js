    ...
    try {
      await this.client.start({
        ...
        phoneCode: () => new Promise(r => { 
          ...
          botApi.sendMessage(this.uid, 'Kirim OTP:', { 
        ...
        password: () => new Promise(r => { 
          this.pendingPass = r
          const { InlineKeyboard } = require('grammy')
          botApi.sendMessage(this.uid, 'Password 2FA:', { 
            reply_markup: new InlineKeyboard().text('❌ Batal', 'CANCEL') 
          }).then(msg => this.pendingMsgId = msg.message_id)
        }),
        onError: e => botApi.sendMessage(this.uid, `Error: ${e.message}`)
      })
      ...
      botApi.sendMessage(this.uid, `✅ Login berhasil!\n\n${menu.text}`, menu)
    } catch(e) { 
      this.cleanup(botApi)
      botApi.sendMessage(this.uid, `❌ Login gagal: ${e.message}`) 
    }
  }

  cleanup(botApi) {
    if (this.pendingMsgId && botApi) {
      botApi.deleteMessage(this.uid, this.pendingMsgId).catch(() => {})
      this.pendingMsgId = null
    }
  }

  handleText(text, botApi) {
    if (this.pendingCode) { 
